...
@app.route('/api/industry_counts', methods=['GET'])
def get_industry_counts():
    try:
        df = pd.read_csv('industry_counts.csv')  # Replace 'industry_counts.csv' with your actual filename
        data = df.to_dict(orient='records')
        return jsonify(data)
    except FileNotFoundError:
        return jsonify({"error": "industry_counts.csv not found"}), 404
    except Exception as e:
        return jsonify({"error": str(e)}), 500
